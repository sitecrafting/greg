...
# s/greg/<your-project-name>/ to finish setting up.
...
function main() {
  if ! [[ -f ./.lando.yml ]] ; then
    fail 'Error: not in root greg directory?'
  fi
  ...
  # hackishly create a symlink greg directory, so that when extracted, the
